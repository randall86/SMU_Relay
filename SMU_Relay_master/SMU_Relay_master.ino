...
void loop() {
    ...
    if (Serial.available())
    {
        ...
        if (('!' <= tmp_char) && (tmp_char <= '~'))
        {
            ...
            if ( ((delim_count == 4) || (delim_count == 8)) && 
                (cmd_idx > delim_idx[delim_count]) &&
                (cmd_str[delim_idx[delim_count]+1] == END_CHAR) )
            {
                        ...
                        if (cmd_str[delim_idx[1]+1] == RELAY_CHAR)
                        {
                            ...
                            else
                            {
                                ...
                                else if (delim_count == 8)
                                {
                                    ...
                                    for (int i = 0; i < 4; i++)
                                    {
                                        RELAY_num[i] = atoi(&cmd_str[delim_idx[i+2]+1]);
                                        
                                        if ( (RELAY_num[i] >= 1) && (RELAY_num[i] <= 16) ) // only supports RELAY 1-16
                                        {
                                            //Serial.println("ERROR: unknown RELAY");
                                            is_err = true;
                                            printReply(NACK_STR);
