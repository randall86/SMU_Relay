...
void relayWritePortWrapper(relay_port_map_t *p_relay, byte state)
{
    if (NULL != p_relay)
    {
        multiplexer_U1.selectChannel(p_relay->muxNum);
...
#endif
    {
        ...
        if (('!' <= tmp_char) && (tmp_char <= '~'))
        {
                ...
                (cmd_str[delim_idx[delim_count-1]+1] == END_CHAR) )
            {
                ...
                {
                    //check for ON/OFF
                    byte state = (cmd_str[delim_idx[1]+1] == ON_CHAR) ? 0xFF : 0x00;
                ...
                else if (cmd_str[0] == RELAY_CHAR)
                {
                    if (cmd_str[delim_idx[0]+1] == RESET_CHAR)
                    {
                        ...
                        if (cmd_str[delim_idx[1]+1] == ON_CHAR)
                        {
                            ...
                            for (int i = 0; i < 16; i++)
                            {
                                relayWritePortWrapper(&relay_port_map[i], 0xFF);
                            }
                        ...
                        else if (cmd_str[delim_idx[1]+1] == OFF_CHAR)
                        {
                            ...
                            for (int i = 0; i < 16; i++)
                            {
                                relayWritePortWrapper(&relay_port_map[i], 0x00);
                            }
